...
$app->get('/params/{id}', function (Request $request, Response $response, $args) {
    ...
    if($tree){
        if($tree == 'GROUPS AND OBJECTS'){
            ...
            for ($i = 0; $i < count($rows); $i++) {
                if($rows[$i]['node_type'] == 1){
                    $rows[$i]['count_of_indicators'] = count(DB::fetchAll("SELECT * FROM indicators WHERE object_id=".$rows[$i]['id'].";"));
                }
                else{
                    $rows[$i]['count_of_child_groups'] = count(DB::fetchAll("SELECT * FROM object_tree WHERE left_key > ".$rows[$i]['left_key'].
                        " AND right_key < ".$rows[$i]['right_key']." AND node_type=0;"));
                    $rows[$i]['count_of_child_objects'] = count(DB::fetchAll("SELECT * FROM object_tree WHERE left_key > ".$rows[$i]['left_key'].
        ...
        $rowGroup = DB::fetchAll("SELECT *, IF((SELECT COUNT(*) FROM users u WHERE u.id = s.user_id) <> 0, ".
            "(SELECT CONCAT(first_name, ' ', second_name) FROM users u WHERE u.id = s.user_id), 'невідомий') AS user ".
            "FROM object_tree s WHERE node_type=0 AND id = $groupID;");
...
$app->post('/params', function (Request $request, Response $response, $args) {
    ...
    $parentGroupInfo = DB::fetchAll("SELECT * FROM object_tree WHERE id = $parentGroupID;");
    ...
    if($isCopy){
        ...
        $groupData = DB::fetchAll("SELECT * FROM object_tree WHERE left_key >= $groupLeftKey AND right_key <= $groupRightKey;");

        $sql = "UPDATE object_tree SET left_key = left_key + $diff, right_key = right_key + $diff " .
            "WHERE left_key > $parentGroupRightKey;";
        file_put_contents('_SQL_1.txt', $sql);
        DB::exec($sql);

        //оновлюємо батьківську гілку
        $sql = "UPDATE object_tree SET right_key = right_key + $diff " .
        ...
        for($i = 0; $i < count($groupData); $i++){
            //додаємо новий вузол
            $sql = "INSERT INTO object_tree SET left_key = ".($parentGroupRightKey + ($groupData[$i]['left_key'] - $groupLeftKey))
            ...
            if($groupData[$i]['node_type'] == 1) {
                $newObject = DB::fetchAll("SELECT * FROM object_tree WHERE left_key = " . ($parentGroupRightKey + ($groupData[$i]['left_key'] - $groupLeftKey))
                    . " AND right_key = " . ($parentGroupRightKey + ($groupData[$i]['right_key'] - $groupLeftKey)) . ";");

                $newObjectID = $newObject[0]['id'];
                $indicators = DB::fetchAll("SELECT * FROM indicators WHERE object_id = ".$groupData[$i]['id'].";");

                for ($j = 0; $j < count($indicators); $j++) {
                    $sql = "INSERT INTO indicators (object_id, name, description, user_id) " .
                        "VALUES ($newObjectID, '" . $indicators[$j]['name'] . "', '" . $indicators[$j]['description'] . "', $userID, creation_date = '$now');";
                    DB::exec($sql);
                }
            }
        }
    ...
    //створення групи
        ...
        //додаємо новий вузол
        $sql = "INSERT INTO object_tree SET left_key = $parentGroupRightKey, right_key = " .
...
$app->put('/params/{id}', function (Request $request, Response $response, $args) {
    ...
    if($isMove) {
        ...
        $group = DB::fetchAll("SELECT * FROM object_tree WHERE id = $groupID;");
        ...
        $sql = "UPDATE object_tree SET name = '$groupName', description='$groupDescription' WHERE id = $groupID;";
        DB::exec($sql);

        //оновлюємо новий вузол
        //коли переміщаємо лівіше
        if($parentGroupRightKey < $oldGroupLeftKey){
            $sql = "UPDATE object_tree SET left_key=left_key-" . ($oldGroupLeftKey + $diff). "+" . $parentGroupRightKey .
            ...
            $sql = "UPDATE object_tree SET right_key = right_key - $diff WHERE right_key > ($oldGroupRightKey + $diff);";
            DB::exec($sql);

            $sql = "UPDATE object_tree SET left_key = left_key - $diff WHERE left_key > ($oldGroupRightKey + $diff);";
            DB::exec($sql);
        }
        //коли переміщаємо правіше
        else {
            $sql = "UPDATE object_tree SET left_key=left_key-" . $oldGroupLeftKey . "+" . $parentGroupRightKey .
            ...
            //оновлюємо ключі від попереднього вузла
            $sql = "UPDATE object_tree SET right_key = right_key - $diff WHERE right_key > $oldGroupRightKey;";
            DB::exec($sql);

            $sql = "UPDATE object_tree SET left_key = left_key - $diff WHERE left_key > $oldGroupRightKey;";
            DB::exec($sql);
        }
    }
...
$app->delete('/params/[{id}]', function (Request $request, Response $response, $args) {
    ...
    //отримуємо список підлеглих об'єктів
    $objects = DB::fetchAll("SELECT * FROM object_tree WHERE left_key >= $left_key AND right_key <= $right_key AND node_type = 1;");

    //видаляємо індикатори для підлеглих об'єктів
    for ($i = 0; $i < count($objects); $i++) {
        //видалення дій над даними індикаторами, що входять до даного об'єкта
        $indicators = DB::fetchAll("SELECT * FROM indicators WHERE object_id = ".$objects[$i]['id'].";");
        for($j = 0; $j < count($indicators); $j++){
            $sql = "DELETE FROM actions WHERE item_id = ".$indicators[$j]['id'].";";
            DB::exec($sql);
        }
        //видаляємо індикатори об'єкта
        $sql = "DELETE FROM indicators WHERE object_id = ".$objects[$i]['id'].";";
        DB::exec($sql);
    }
    ...
    if($item[0]['node_level'] == 1){
        ...
        DB::exec($sql);
        //видаляємо групи та об'єкти
        $sql = "DELETE FROM object_tree WHERE left_key > $left_key AND right_key < $right_key;";
        DB::exec($sql);
        $sql = "UPDATE object_tree SET left_key = 1, right_key = 2 WHERE id = $id;";
        DB::exec($sql);
    }
    else {
        //видаляємо дії над підлеглими компетнціями та групами
        $objectsAndGroups = DB::fetchAll("SELECT * FROM object_tree WHERE left_key >= $left_key AND right_key <= $right_key;");
        for($i = 0; $i < count($objectsAndGroups); $i++){
            $sql = "DELETE FROM actions WHERE item_id = ".$objectsAndGroups[$i]['id']." OR new_parent_id = ".$objectsAndGroups[$i]['id'].";";
            DB::exec($sql);
        }
        //видаляємо групи та об'єкти
        $sql = "DELETE FROM object_tree WHERE left_key >= $left_key AND right_key <= $right_key;";
